...
jQuery(document).ready(function ($) { 
    ...
    const search_tool_type = $("#IG_search_tool_for_influencer").data('type');
    ...
    const posttype= $("#influencers_card_grid").data('type');
 
    
    // handeling available free searches logic
    
    let free_searches = getCookie(search_tool_type);

    if(free_searches === null){ // when a user is searching for the first time in a day
        setCookie(search_tool_type, 5, 1);
        free_searches = getCookie(search_tool_type);
    }
    let available_free_searches = free_searches;

    search_button.html(`Search Influencers (${available_free_searches})`);
    const endpoint= 'https://search-api.influencers.club/search/ig/';
    
    // --------------------------------------------
    const getInfluencers = (form_values) => {
        ...
        .then((response) => {
            ...
            // whatever response comes, enable the search button for the next search
        ...
        .then((res_data) => {
            ...
            if(data_count > 0){
                show_result_div.html(`<h5>Showing ${data_count>=6 ? "6" : data_count} of ${random_number}k profiles. To see all results (with even more data), <a href="https://influencers.club/signup">start a free trial.</a></h5>`);
                ...
                for (let index = 0; index < index_end; index++) {
                    ...
                    <div class="IG-tool-profile-card ${card_colors[index%4]}">
                        <div class="IG-tool-profile-head">
                            <i> <img src=${influencer.instagram.profile_picture ? influencer.instagram.profile_picture : localized_data.userplaceholder} alt="Check"> </i>
                            <h6>
                                ${influencer.first_name !== null ? influencer.first_name : ("@" + influencer.instagram.username)} 
                    ...
                    if(influencer.external_url?.length > 0){
                        influencer_card += `
                            <li> <a href="${influencer.external_urls[0]}" target="_blank"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    influencer_card += `
                            ...
                            if(influencer.instagram.follower_count){
                                ...
                                <li>
                                    <span>Fans</span>
                                    ${shortenNumberFormatfortool(influencer.instagram.follower_count)}
                                </li>
                                `;
                            }    
                        ...
                        if(search_tool_type === 'influencer_search'){
                            influencer_card += `
                                ...
                                influencer.instagram.hashtags ? `
                                    <ul>
                                    ${
                                        influencer.instagram.hashtags.filter((hashtag) => {return hashtag !== "]" && hashtag !== "["})
                                            .map(hashtag => `<li>#${hashtag}</li>`).join('')
                                    }
                                    </ul>
                                    ` : ''
                                }
                            ...
                            influencer_card += `<p class="insta-bio">${influencer.instagram.biography ? influencer.instagram.biography : ""}</p>`;
                        }
                    ...
                    search_button.prop("disabled", false);
                    setCookie(search_tool_type, available_free_searches, 1);
                }
                    ...
                    <div class="IG-tool-notify notFound-section">
                        <div class="inner-content">
                            ...
                            <ul id="not_found_search_options">
                                <li data-value="Crypto" class="sample-search" > <b>Crypto</b></li>
                                <li data-value="Fashion Influencer" class="sample-search"> <b>Fashion Influencer</b></li>
                                <li data-value="Pet Owner" class="sample-search"> <b>Pet Owner</b></li>
                            </ul>
                        </div>
                    </div>
                `);
    ...
    const sampleSearchOptionsClick = (ul_list_id) => {
        $(`#${ul_list_id} li`).on('click', (e) => {
            ...
            $('#bio').val(keyword_sample);
            
            const form_values = { bio: keyword_sample};

            if(available_free_searches > 0){
                getInfluencers(form_values);
            } else {
                show_result_div.html(out_of_searches_message);
            }

        });
    }

    if(search_tool_type === "ig_search"){
        $('#ig_search_sample_options').html(`
        <div class="tool-ig-sample-searches">
            <p>Try it out!</p>
            <ul id="ig_search_options">
                <li data-value="Crypto" class="sample-search" >Crypto</li>
                <li data-value="Fashion Influencer" class="sample-search">Fashion Influencer</li>
                <li data-value="Pet Owner" class="sample-search">Pet Owner</li>
                <li data-value="Blogger" class="sample-search">Blogger</li>
                <li data-value="Writter" class="sample-search">Writter</li>
                <li data-value="Videographer" class="sample-search">Videographer</li>
                <li data-value="Freelancer" class="sample-search">Freelancer</li>
                <li data-value="Food Creator" class="sample-search">Food Creator</li>
            </ul>
        </div>
        `);
        sampleSearchOptionsClick('ig_search_options');
    }
